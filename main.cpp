...
bool CheckImages(const std::vector<Problem> &problems) {
    ...
    for (size_t i = 1; i < problems.size(); ++i) {
        ...
        if (image.cols != iw || image.rows != ih) {
            return false;
        }
    }
    return true;
...
int CountRoundNum(const std::vector<Problem> &problems) {
    if (problems.size() == 0) {
        return 0;
    }
    path image_path = problems[0].dense_folder / path("images") / path(ToFormatIndex(problems[0].ref_image_id) + ".jpg");
...
void SampleList(std::vector<Problem> &problems,const path &dense_folder)
{
    ...
    for (int i = 0; i < num_images; ++i) {
        ...
        problem.result_folder = dense_folder / path("TTR") / path(ToFormatIndex(problem.ref_image_id));
...
void ProcessProblem(const Problem &problem) {
    ...
    TTR TTR(problem);
    TTR.InuputInitialization();
    TTR.CudaSpaceInitialization();
    TTR.SetDataPassHelperInCuda();
    TTR.RunPatchMatch();

    int width = TTR.GetWidth(), height = TTR.GetHeight();
    cv::Mat depth = cv::Mat(height, width, CV_32FC1);
    cv::Mat normal = cv::Mat(height, width, CV_32FC3);
    cv::Mat pixel_states = TTR.GetPixelStates();
    for (int r = 0; r < height; ++r) {
        for (int c = 0; c < width; ++c) {
            float4 plane_hypothesis = TTR.GetPlaneHypothesis(r, c);
            depth.at<float>(r, c) = plane_hypothesis.w;
            if (depth.at<float>(r, c) < TTR.GetDepthMin() || depth.at<float>(r, c) > TTR.GetDepthMax()) {
    ...
    WriteBinMat(selected_view_path, TTR.GetSelectedViews());
    ...
    if (problem.show_medium_result) {
        ...
        ShowDepthMap(depth_img_path, depth, TTR.GetDepthMin(), TTR.GetDepthMax());
...
int main(int argc, char **argv) {
    if (argc < 2) {
        std::cerr << "USAGE: TTR dense_folder\n";
        return EXIT_FAILURE;
    }
    path dense_folder(argv[1]);
    path output_folder = dense_folder / path("TTR");
    ...
    SampleList(problems,dense_folder);
    ...
    int round_num = CountRoundNum(problems);
    ...
    //Runfusion functions (Intermediate and Advance are both for T&T dataset, original RunFusion is for eth3d dataset)
    //RunFusion_TAT_Intermediate(dense_folder, problems);
    //RunFusion_TAT_advanced(dense_folder, problems);
