...
__device__ int compareTextureSize(size_t allocated_size, int width, int height) {
    size_t expected_size = width * height * sizeof(float);
    if(allocated_size == expected_size){
        return 1; //equal
    }
    else if(allocated_size > expected_size){
        return 2; //too large,downsample
    }
}

__device__ void downsample_texture(size_t allocated_size, float* tex_d, float* down_td, int inputWidth, int inputHeight) {
    int factor = sqrt((float)allocated_size / (inputWidth * inputHeight)); // 假设下采样因子是平方的
    int new_w = inputWidth / factor;
    int new_h = inputHeight / factor;
    for (int i = 0; i < new_h; ++i) {
        for (int j = 0; j < new_w; ++j) {
            int src_x = j * factor;
            int src_y = i * factor;
            int dest_idx = i * new_w + j;
            int src_idx = src_y * inputWidth + src_x;
            down_td[dest_idx] = tex_d[src_idx];
        }
    }
}
...
#if TEXTURE_TEXTURE_STRONG
__device__ void CheckerboardPropagationStrong_texture(
        const int2 p,
        const int iter,
        DataPassHelper *helper,
        float* texture_sampling
) {
    const int width = helper->width;
    const int height = helper->height;
    float4 *plane_hypotheses = helper->plane_hypotheses_cuda;
    ...
    //TEXTURE_TEXTURE_WEAK
    float combined_cost = 0;
    float texture_costs=0;
    //
    float costMin;
    int costMinPoint;
    ...
    //c
    allocated_texture_size = width * height * sizeof(float);
    int comp_flag = compareTextureSize(allocated_texture_size,width,height);
    //printf("comp_flag=%d",comp_flag);
    if(comp_flag==1){
        texture_costs = GetTextureValue(texture_sampling,p,width,height);
        //printf("texture_costs=%f   ",texture_costs);
    }
    else if(comp_flag==2){
        printf("w*h = %d * %d, texturesize = %d\n",width,height,allocated_texture_size);
        float* down_textureCost;
        downsample_texture(allocated_texture_size,texture_sampling,down_textureCost,width,height);
    }

    float sampling_probs[32] = { 0.0f };
    //
    float cost_threshold = 0.8 * expf((iter) * (iter) / (-30.0f));
    ...
    for (int i = 0; i < num_images - 1; i++) {
        ...
        for (int j = 0; j < 8; j++) {
            if(texture_costs<=5){
                combined_cost = cost_array[j][i] +(texture_costs-2)*0.02;
            }
            else if(texture_costs>5 && texture_costs<=10){//0.3{
                combined_cost = cost_array[j][i] -0.7*log(texture_costs-4)-0.06;
            }
            else if(texture_costs>10){//0.3{
                combined_cost = cost_array[j][i] -0.4*log(texture_costs-4)-0.21;
            }
            //printf("cost_array[j][i]: %f combined_cost: %f texture_costs: %f \n",cost_array[j][i],combined_cost,texture_costs);
            //cost_array[j][i] < cost_threshold
            //printf("cost_array[j][i] %f   ",cost_array[j][i]);
            if (combined_cost < cost_threshold) {
            //if (cost_array[j][i] < cost_threshold) {
                tmpw += expf(cost_array[j][i] * cost_array[j][i] / (-0.18f));
                count++;
            }
            if (combined_cost > 1.2f) {
                count_false++;
            }
        }
...
#if TEXTURE_TEXTURE_WEAK
__device__ void CheckerboardPropagationWeak_texture(
	const int2 p,
	const int iter,
	DataPassHelper *helper,
    float* texture_sampling
	...
	for (int i = 0; i < num_images - 1; i++) {
		...
		for (int j = 0; j < 8; j++) {
            if(texture_costs<=5){
                ...
                combined_cost = cost_array[j][i] -0.1*log(texture_costs-4)-0.06;
            }
            else if(texture_costs>10){//0.3{
                combined_cost = cost_array[j][i] -0.3*log(texture_costs-4)+0.1;
            }
            //printf("cost_array[j][i]: %f combined_cost: %f texture_costs: %f \n",cost_array[j][i],combined_cost,texture_costs);
            //cost_array[j][i] < cost_threshold
            //printf("cost_array[j][i] %f   ",cost_array[j][i]);
	...
	for (int i = 0; i < 8; ++i) {
		for (int j = 0; j < num_images - 1; ++j) {
			if (view_weights[j] > 0) {
				if (params->geom_consistency) {
					if (flag[i]) {
						final_costs[i] += view_weights[j] * (cost_array[j][i] + params->geom_factor * ComputeGeomConsistencyCost(p, j + 1, plane_hypotheses[positions[i]], helper));
					}
					else {
						final_costs[i] += view_weights[j] * (cost_array[j][i] + params->geom_factor * 3.0f);
					}
				}
				else {
					final_costs[i] += view_weights[j] * cost_array[j][i];
				}
			}
		}
...
#if TEXTURE_TEXTURE_WEAK
__global__ void BlackPixelUpdateWeak_texture(const int iter, DataPassHelper *helper,float* texture_sampling)
{
    ...
    if (helper->weak_info_cuda[p.x + p.y * helper->width] == WEAK) {
        ...
        CheckerboardPropagationWeak_texture(p, iter, helper,texture_sampling);
    }
}
#endif
...
#if TEXTURE_TEXTURE_WEAK
__global__ void RedPixelUpdateWeak_texture(const int iter, DataPassHelper *helper,float* texture_sampling)
{
	int2 p = make_int2(blockIdx.x * blockDim.x + threadIdx.x, blockIdx.y * blockDim.y + threadIdx.y);
...
#if TEXTURE_TEXTURE_STRONG
__global__ void BlackPixelUpdateStrong_texture(const int iter, DataPassHelper *helper,float* texture_sampling)
{
    int2 p = make_int2(blockIdx.x * blockDim.x + threadIdx.x, blockIdx.y * blockDim.y + threadIdx.y);
    ...
    CheckerboardPropagationStrong_texture(p, iter, helper,texture_sampling);
}

__global__ void RedPixelUpdateStrong_texture(const int iter, DataPassHelper *helper,float* texture_sampling)
{
    int2 p = make_int2(blockIdx.x * blockDim.x + threadIdx.x, blockIdx.y * blockDim.y + threadIdx.y);
...
__global__ void DepthToWeak_texture(DataPassHelper *helper, float* texture_sampling) {
    const int2 point = make_int2(blockIdx.x * blockDim.x + threadIdx.x, blockIdx.y * blockDim.y + threadIdx.y);
    const int width = helper->width;
    const int height = helper->height;
    //TEXTURE_BASED_SAMPLING
    float texture_cost = 0;
    texture_cost = GetTextureValue(texture_sampling,point,width,height);
    if (point.x >= width || point.y >= height) {
        return;
    }
...
#if TEXTURE_TEXTURE_STRONG
        BlackPixelUpdateStrong_texture << <grid_size_half, block_size_half >> > (i, helper_cuda, texture_obj_sampling_cuda);
#else
        BlackPixelUpdateStrong << <grid_size_half, block_size_half >> > (i, helper_cuda);
#endif
		CUDA_SAFE_CALL(cudaDeviceSynchronize());
#if TEXTURE_TEXTURE_STRONG
        RedPixelUpdateStrong_texture << <grid_size_half, block_size_half >> > (i, helper_cuda, texture_obj_sampling_cuda);
#else
        RedPixelUpdateStrong << <grid_size_half, block_size_half >> > (i, helper_cuda);
#endif

		CUDA_SAFE_CALL(cudaDeviceSynchronize());
		//std::cout << "Iteration " << i << " complex texture area done\n";
		RANSACToGetFitPlane << <grid_size_full, block_size_full >> > (helper_cuda);
		CUDA_SAFE_CALL(cudaDeviceSynchronize());
#if TEXTURE_TEXTURE_WEAK
        BlackPixelUpdateWeak_texture << <grid_size_half, block_size_half >> > (i, helper_cuda, texture_obj_sampling_cuda);
#else
        BlackPixelUpdateWeak << <grid_size_half, block_size_half >> > (i, helper_cuda);
#endif
		CUDA_SAFE_CALL(cudaDeviceSynchronize());
#if TEXTURE_TEXTURE_WEAK
        RedPixelUpdateWeak_texture << <grid_size_half, block_size_half >> > (i, helper_cuda, texture_obj_sampling_cuda);
#else
        RedPixelUpdateWeak << <grid_size_half, block_size_half >> > (i, helper_cuda);
#endif
		CUDA_SAFE_CALL(cudaDeviceSynchronize());
		std::cout << "Iteration " << i << " complex & low texture areas done\n";
	}
