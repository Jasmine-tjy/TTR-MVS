...
bool ReadCamera(const path &cam_path, Camera &cam)
{
	...
	float depth_num; //short camera change to float depth_num=256;
	float interval;
    //in >> cam.depth_min >> interval ;//short camera
    in >> cam.depth_min >> interval >> depth_num >> cam.depth_max;
    //cam.depth_max = interval * depth_num + cam.depth_min; //short camera
	// ====================================================================
	////DTU version read
	//float depth_num = 192;
	//float interval;
...
TTR::~TTR() {
    ...
    cudaFree(texture_obj_sampling_cuda);

}

bool readTextureCsv(const std::string& filename, float* textureValue, int rows, int cols) {
    std::ifstream file(filename);
    if (!file.is_open()) {
        //std::cerr << "Error opening origin csv file: " << filename << std::endl;
        if (file.fail()) {
            std::cerr << "failed: " << file.rdstate() << std::endl;
        }
        return false; // File opening failed, nothing to read
    }
    int index = 0;
    std::string line;
...
float bilinearInterpolate(const std::vector<std::vector<float>>& input, float x, float y) {
    int x1 = static_cast<int>(x);
    int y1 = static_cast<int>(y);
    int x2 = x1 + 1;
    int y2 = y1 + 1;

    float q11 = input[y1][x1];
    float q21 = (x2 < input[0].size()) ? input[y1][x2] : q11;
    float q12 = (y2 < input.size()) ? input[y2][x1] : q11;
    float q22 = (x2 < input[0].size() && y2 < input.size()) ? input[y2][x2] : q11;
...
void upsampleTexture(const std::vector<std::vector<float>>& input, float* output, int outputWidth, int outputHeight) {
    int inputHeight = input.size();
    int inputWidth = input[0].size();

    for (int i = 0; i < outputHeight; ++i) {
        for (int j = 0; j < outputWidth; ++j) {
            float x = static_cast<float>(j) * (inputWidth - 1) / (outputWidth - 1);
            float y = static_cast<float>(i) * (inputHeight - 1) / (outputHeight - 1);
            output[i * outputWidth + j] = bilinearInterpolate(input, x, y);
        }
    }
}

void upsampleTextureCsv(const std::string& filename, float* textureValue, int Orirows, int Oricols, int Newrows, int Newcols) {
    std::ifstream file(filename);
    if (!file.is_open()) {
    ...
    std::vector<std::vector<float>> originalData(Orirows, std::vector<float>(Oricols));
    int index = 0;
    std::string line;
    while (std::getline(file, line) && index < Orirows * Oricols) {
        std::istringstream lineStream(line);
        std::string cell;
        int colIndex = 0;
        while (std::getline(lineStream, cell, ',') && colIndex < Oricols) {
            float value;
            if (std::stringstream(cell) >> value) {
                originalData[index / Oricols][colIndex] = value;
            } else {
                std::cerr << "Error converting string to float: " << cell << std::endl;
            }
            colIndex++;
    ...
    if (index != Orirows * Oricols) {
        std::cerr << "Warning: CSV file has fewer elements than expected." << std::endl;
    }
    std::cout<<""<<std::endl;
    upsampleTexture(originalData, textureValue, Newcols, Newrows);
}


void TTR::InuputInitialization() {
	images.clear();
	cameras.clear();
	...
	//std::cout << "----------------------- Processing Image " << params_host.num_images << " -----------------------"<<std::endl;
	// =================================================
	// scale images
	if (problem.scale_size != 1) {
		for (int i = 0; i < num_images; ++i) {
            ...
            if(i==0){
                ...
                path result_file_folder=problem.dense_folder / path("/TTR/")/ path(ToFormatIndex(problem.ref_image_id));
                //image
                ...
                //    continue;
                //}
                //std::string texture_IMG_suffix = "/texture_" + std::to_string(factor) + ".jpg";
                //std::string texture_IMG_path = result_file_folder.string() + texture_IMG_suffix;
                //cv::imwrite(texture_IMG_path, convertedImage);

                std::string texture_MAT_suffix = "/texture_" + std::to_string(new_rows)+"x" + std::to_string(new_cols) + ".csv";
                std::string texture_MAT_path = result_file_folder.string() + texture_MAT_suffix;
                ...
                if (access(texture_MAT_path.c_str(), F_OK) != -1) {
                    ...
                    //std::cout << "Pre Weak count: " << pre_weak_count << " / " << new_rows * new_cols << " = " << (float)pre_weak_count / (float)(new_rows * new_cols) * 100 << "%" << std::endl;
	...
	if (params_host.geom_consistency) {
		...
		for (const auto &src_idx : problem.src_image_ids) {
			path src_depth_path = problem.dense_folder / path("TTR") / path(ToFormatIndex(src_idx)) / path("depths.dmb");
			cv::Mat src_depth;
	...
	if (params_host.use_TTR) {
		...
		if (weak_info_host.cols != width || weak_info_host.rows != height) {
			std::cerr << "Rescale Weak support images' size!\n";
	...
	if (params_host.state != FIRST_INIT) {
		...
		if (depth.cols != width || depth.rows != height || normal.cols != width || normal.rows != height) {
			std::cerr << "Rescale Depth and Normal map!\n";
		...
		}
		{
			...
			if (selected_views_host.cols != width || selected_views_host.rows != height) {
				std::cerr << "Rescale Select view images' size!\n";
...
void TTR::CudaSpaceInitialization() {
	// =================================================
	// move images to gpu
	for (int i = 0; i < num_images; ++i) {
        ...
        if (i == 0) {
            ...
            result_path << problem.dense_folder.string() << "/TTR/" << std::setw(8) << std::setfill('0') << problem.ref_image_id;
            std::string result_folder = result_path.str();
            std::string texture_MAT_suffix = "/texture_" + std::to_string(cameras[0].height) + "x" + std::to_string(cameras[0].width) + ".csv";
            ...
            csvFlag = readTextureCsv(texture_MAT_path,texture_sampling_value,cameras[0].height,cameras[0].width);
            if(csvFlag==false){
                int ori_height = static_cast<int>((static_cast<float>(cameras[0].height)/2) + 0.5f);
                int ori_width = static_cast<int>((static_cast<float>(cameras[0].width)/2) + 0.5f);
                std::string texture_MAT_suffix_up = "/texture_" + std::to_string(ori_height) + "x" + std::to_string(ori_width) + ".csv";
                std::string texture_MAT_path_up = result_folder + texture_MAT_suffix_up;
                upsampleTextureCsv(texture_MAT_path_up,texture_sampling_value,ori_height,ori_width,cameras[0].height,cameras[0].width);
                printf("upsampling texture...");
            }
...
void TTR::SetDataPassHelperInCuda() {
...
float4 TTR::GetPlaneHypothesis(int r, int c) {
	return plane_hypotheses_host[c + r * width];
}

cv::Mat TTR::GetPixelStates() {
	return weak_info_host;
}

cv::Mat TTR::GetSelectedViews() {
...
float TTR::GetDepthMin() {
	return params_host.depth_min;
}

float TTR::GetDepthMax() {
...
void RunFusion(const path &dense_folder, const std::vector<Problem> &problems)
{
	...
	path ply_path = dense_folder / path("TTR") / path("TTR.ply");
